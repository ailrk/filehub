...
emit :: (ToLogEvt evt, MonadIO m) => EvtLog -> evt -> m ()
emit EvtLog { conn, maxSize } evt = liftIO do
...
fold :: (FromLogEvt evt, MonadIO m)
     ...
     -> (proj -> evt -> proj) -> proj
     -> m proj
fold EvtLog { conn } mRange f proj = liftIO go
  where
    go = case mRange of
           Just (from, to) -> do
             Sqlite.fold conn
...
initialize :: MonadIO m => String -> Int -> m EvtLog
...
close :: MonadIO m => EvtLog -> m ()
close EvtLog { conn } = liftIO do
  Sqlite.close conn
